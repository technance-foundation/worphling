...
export class OpenAiTranslationProvider implements TranslationProviderContract {
    ...
    #buildSystemPrompt(exactLength: boolean, contextInstructions?: string): string {
        ...
        return [
            ...
            "Never remove required ICU branches from the source message, including `other` and exact-match selectors such as `=0`.",
            "You may add extra locale-appropriate plural categories only when needed by the target language, but never remove source-required branches.",
            "Preserve placeholders exactly when present.",
            "Never invent new placeholders.",
            "Never remove placeholders.",
            "Never replace placeholders with translated words or hardcoded literals.",
            "Placeholders and tags may move to a different position in the sentence when needed for natural translation, but they must still be preserved exactly.",
            "Preserve HTML-like or rich-text tags exactly when present.",
            "Never rename tags.",
            "Never remove tags.",
            "Never replace tags with plain text or markdown.",
            "Rich-text tags may move to a different position in the sentence when needed for natural translation, but the same tag structure must still exist.",
            "Do not translate placeholder names, ICU selectors, tag names, or JSON keys.",
